...
def parse_config_file(file_path):
    ...
    """
    config_data = {}
    # Provided regex pattern to match key-value pairs
    pattern = r'^([A-Z_][A-Z0-9_:${}]*)\s*=\s*"([^"]*(?:\\[\s\S]*?)*)"'

    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                # Skip empty lines and lines starting with 'inherit' or function definitions
                line = line.strip()
                if not line or line.startswith('inherit') or line.startswith('do_'):
                    continue
...
if __name__ == "__main__":
    ...
    for _key in result:
        ...
        assert _key != "inherit", (
            "inherit statement should not be parsed as a key")
